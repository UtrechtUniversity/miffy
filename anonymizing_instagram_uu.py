...
def _main_folder():
    ...
    # Files in directory of python script
    list_of_files = os.listdir() #list of files in the current directory
    
    # List of all instagram packages
    folder = []
    for each_file in list_of_files:
        if each_file.endswith(".zip"):  #since its all type str you can simply use startswith
            folder.append(each_file)
...
def _main_files(n):
    ...
    # Path to instagram package
    data = project + '\\' + str(_main_folder()[n])
    
    # New folder where unpacked files will be stored
    new_folder = data.replace(".zip", "") 
    new_folder = re.sub(r"\d+", "", new_folder)
    new_folder = new_folder.replace("_", "")
    
    return([project, data, new_folder])
...
def main_unpack():
    for n in range(len(_main_folder())):    
        ...
        file_name = _main_files(n)[1]
        print('Extracting all files from '+ f'{_main_files(n)[1]}' + ' (' + f'{n+1}' + ' of ' + f'{len(_main_folder())}' + ' packages)') 
        
        with ZipFile(file_name, 'r') as zip: 
            zip.extractall(f'{_main_files(n)[2]}') 
            print('Done!') 
