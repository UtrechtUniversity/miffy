...
def read_participants(input_folder:str):
    """ Open file with all participant numbers """    
    
    path = Path(input_folder) / 'participants.csv'
...
def extr_usernames(df, input_folder:str):
    ...
    participants = read_participants(input_folder)
    dictionary = {}
...
def extr_names(df, input_folder:str):
...
def create_keys(input_folder:str, output_folder:str):
    ...
    for folder in folders:
        ...
        dictionary = extr_usernames(df, input_folder)
        dictionary.update(extr_names(df, input_folder))
        dictionary.update(extr_mail(df))
        dictionary.update(extr_phone(df))
             
        df = pd.DataFrame(list(dictionary.items()))
        df = df.rename(columns={0: 'id', 1: 'subt'})
        
        subt = dictionary[f'{folder.stem}']
        export_path = Path(input_folder, 'keys'+f"_{subt}.csv")
        df.to_csv(export_path, index=False, encoding='utf-8')
        
        print('Done! See ' + 'keys' + f"_{subt}.csv in " + f'{input_folder}' )
        print(' ')
...
def main():
    ...
    create_keys(args.input_folder, args.output_folder)  
